...
from publish import save_summary
...
class AIVideoSummarizer:
    ...
    def generate_summary(self, loader):
        ...
        if st.button("✨ Generate Summary", use_container_width=True, disabled=not self.youtube_url):
            ...
            # Publish as public shareable page
            channel = GetVideo.channel(self.youtube_url)
            thumbnail = GetVideo.thumbnail(self.youtube_url)
            save_summary(self.video_id, self.video_title, self.summary, channel, thumbnail)
            public_url = f"/v/{self.video_id}"
            st.markdown(
                f'<div style="background:#1a1a2e;border:1px solid #6c5ce7;border-radius:8px;padding:12px 16px;margin:16px 0;">'
                f'<span style="color:#8888a0;font-size:13px;">🔗 Shareable link</span><br>'
                f'<a href="{public_url}" target="_blank" style="color:#6c5ce7;font-size:15px;text-decoration:none;">'
                f'{public_url}</a>'
                f'<span style="display:block;font-size:12px;color:#666;margin-top:4px;">'
                f'Run the server: <code>uvicorn server:app --port 8000</code></span>'
                f'</div>',
                unsafe_allow_html=True
            )
